...
class prompting:
    def __init__(self, model="roberta-base"):
        if model == "roberta-base":
            ...
            raise Exception(
                "Select one of the following models: roberta-base, roberta-large, bert, deberta-base, deberta-large, xlm")

    def predict(self, template, verb_h, verb_nh, data):
        plm, tokenizer, model_config, WrapperClass = load_plm(
            self.checkpoint[0], self.checkpoint[1])
        ...
        if isinstance(data, str):
            ...
            raise ValueError(
                'Input data must be either a string or a pandas DataFrame.')
        ...
        with torch.no_grad():
            for batch in data_loader:
                ...
                predictions.extend([classes[p]
                                for p in preds.cpu().numpy().tolist()])
